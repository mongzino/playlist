...
export async function create(client, user_id) {
  ...
  try {
    ...
    console.log(chalk.bgWhiteBright(" ▶ 플레이리스트가 생성되었습니다!"));
  } finally {
    await getInput();
  }
}
...
export async function find(client, playlist) {
  ...
  try {
    ...
    console.log(chalk.bgWhiteBright(" ▶ 조회를 끝내려면 Enter를 입력해주세요"));
    await getInput();
  }
}
...
export async function modify(client, playlist) {
  ...
  if (sel > playlist.length){
    console.log(chalk.bgWhiteBright("잘못된 입력입니다."));
    await getInput();
    return;
  }
  console.clear();
  ...
  if (val === 1) {
    ...
    try {
      ...
      .updateOne({ _id: playlist[sel].music_id[music_sel] },
        ...
        console.log(chalk.bgWhiteBright("수록곡이 삭제되었습니다."));
    } catch {
      console.log(chalk.bgWhiteBright("잘못된 입력입니다."));
    } finally {
      await getInput();
...
export async function remove(client, playlist) {
  try {
    ...
    console.log(chalk.bgWhiteBright("플레이리스트가 삭제되었습니다."));
  } catch {
    console.log(chalk.bgWhiteBright("잘못된 입력입니다."));
    await getInput();
  }
